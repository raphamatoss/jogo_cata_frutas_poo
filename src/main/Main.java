package main;

import interfaceVisual.telas.Frame;

public class Main {

	private static Frame frame;

	public static void main(String[] args) {
		frame = new Frame();
	}
}

// Essa parte é destinada aos programadores que venham a alterar parte do código
// use "{@link nome_da_classe}" no javadoc para linkar inline uma classe.
// ->Caso queira usar o método de uma classe, use
// nome_da_classe#metodo(parametros)
// -> Use @see no final do comentário para linkar com alguma parte do documento
// -> Use @return para comentar sobre o retorno.
// -> Use @param para comentar sobre o parametro, diga o nome do parametro após
// @param.
//
//
// => Lembrar que deve ser dado o caminho completo (acho) para que o see e o
// link funcionem.
