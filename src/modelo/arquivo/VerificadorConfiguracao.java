...
/**
 * A classe {@code VerificadorConfiguracao} é responsável por gerenciar e verificar as configurações de um mapa que
...
public class VerificadorConfiguracao {
    ...
    private ArrayList<QuantidadeFrutas> qtdFrutas = new ArrayList<>();
    ...
    /** Randomiza a distribuição de um fruta diversa em uma fruta específica.
     */
    public void distribuirUMAfruta() {
        switch (random.nextInt(6)) {
            case(0):
                laranja++;
                break;
    ...
    /** Remove uma fruta específica aleatória caso uma fruta diversa também seja removida.
     */
    public void removerFrutasDiversas() {
        switch (random.nextInt(6)) {
            case(0):
                if (laranja == 0)
                    removerFrutasDiversas();
